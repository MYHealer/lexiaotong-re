package com.jd.ad.sdk.jad_ir;

import java.util.Iterator;
import java.util.LinkedHashMap;
import java.util.Map;

/* JADX INFO: loaded from: C:\Users\MR\AppData\Local\Temp\yixiaotong-dex\6253208.dex */
public class jad_hu<T, Y> {
    public final Map<T, jad_an<Y>> jad_an = new LinkedHashMap(100, 0.75f, true);
    public long jad_bo;
    public long jad_cp;

    public static final class jad_an<Y> {
        public final Y jad_an;
        public final int jad_bo;

        public jad_an(Y y, int i) {
            this.jad_an = y;
            this.jad_bo = i;
        }
    }

    public jad_hu(long j) {
        this.jad_bo = j;
    }

    public synchronized Y jad_an(T t) {
        jad_an<Y> jad_anVar;
        jad_anVar = this.jad_an.get(t);
        return jad_anVar != null ? jad_anVar.jad_an : null;
    }

    public synchronized void jad_an(long j) {
        while (this.jad_cp > j) {
            Iterator<Map.Entry<T, jad_an<Y>>> it = this.jad_an.entrySet().iterator();
            Map.Entry<T, jad_an<Y>> next = it.next();
            jad_an<Y> value = next.getValue();
            this.jad_cp -= (long) value.jad_bo;
            T key = next.getKey();
            it.remove();
            jad_an(key, value.jad_an);
        }
    }

    public void jad_an(T t, Y y) {
    }

    public int jad_bo(Y y) {
        return 1;
    }

    public synchronized Y jad_bo(T t, Y y) {
        int iJad_bo = jad_bo(y);
        long j = iJad_bo;
        if (j >= this.jad_bo) {
            jad_an(t, y);
            return null;
        }
        if (y != null) {
            this.jad_cp += j;
        }
        jad_an<Y> jad_anVarPut = this.jad_an.put(t, y == null ? null : new jad_an<>(y, iJad_bo));
        if (jad_anVarPut != null) {
            this.jad_cp -= (long) jad_anVarPut.jad_bo;
            if (!jad_anVarPut.jad_an.equals(y)) {
                jad_an(t, jad_anVarPut.jad_an);
            }
        }
        jad_an(this.jad_bo);
        return jad_anVarPut != null ? jad_anVarPut.jad_an : null;
    }
}
