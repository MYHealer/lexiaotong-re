...
public abstract class b {
    ...
    public void init(File file, f fVar, e eVar) {
        ...
        this.mDumpDir = new File(file2, sb.append(str).append(x.A).append(this.mIndex).append(".dump").toString());
        this.mLogcatFile = new File(this.mLogDir, str + x.A + this.mIndex + ".log");
        this.mJavaTraceFile = new File(this.mLogDir, str + x.A + this.mIndex + ".jtrace");
        this.mMemoryInfoFile = new File(this.mLogDir, str + x.A + this.mIndex + ".minfo");
    ...
    void backupLogFiles(File file) throws Throwable {
        ...
        try {
            y.i(file.getParentFile().getParentFile(), sBackupDir);
        } catch (IOException e) {
            com.kwad.sdk.core.d.c.printStackTraceOnly(e);
        }
    }
    ...
    private static String getBaseDir() {
        try {
            Context context = com.kwad.sdk.crash.e.Qd().getContext();
            if (context == null) {
            ...
            if (context != null) {
                ...
                if (TextUtils.isEmpty(processName) || SystemUtil.isInMainProcess(context)) {
                    processName = "main";
                } else if (processName != null && processName.startsWith(context.getPackageName() + x.bQ)) {
                    processName = processName.substring(context.getPackageName().length() + 1);
                }
                return processName + x.A + Process.myPid() + x.A + UUID.randomUUID().toString();
            }
            com.kwad.sdk.core.d.c.e(TAG, "getBaseDir() but context is null");
            return UUID.randomUUID().toString();
        } catch (Throwable th) {
            com.kwad.sdk.core.d.c.W(TAG, Log.getStackTraceString(th));
            return UUID.randomUUID().toString();
        }
    }
}
