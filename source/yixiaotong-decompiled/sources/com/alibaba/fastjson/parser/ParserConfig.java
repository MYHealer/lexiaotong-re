...
public class ParserConfig {
    ...
    private ParserConfig(ASMDeserializerFactory aSMDeserializerFactory, ClassLoader classLoader) {
        ...
        if (aSMDeserializerFactory == null) {
            try {
                if (classLoader == null) {
                    aSMDeserializerFactory = ASMDeserializerFactory.getInstance();
                } else {
    ...
    public ObjectDeserializer createJavaBeanDeserializer(Class<?> cls, Type type) {
        ...
        boolean zCheckName = this.asmEnable;
        boolean z = false;
        if (zCheckName) {
        ...
        if (zCheckName && (aSMDeserializerFactory = this.asmFactory) != null && aSMDeserializerFactory.isExternalClass(cls)) {
            zCheckName = false;
        }
        if (zCheckName) {
            zCheckName = ASMUtils.checkName(cls.getName());
        }
        if (!zCheckName) {
            z = zCheckName;
            break;
        ...
        if (deserializeBeanInfoComputeSetters.getFieldList().size() > 200) {
            zCheckName = false;
        }
        if (deserializeBeanInfoComputeSetters.getDefaultConstructor() == null && !cls.isInterface()) {
            zCheckName = false;
        }
        ...
        while (true) {
            ...
            if (!ASMUtils.checkName(next.getMember().getName())) {
                zCheckName = false;
            }
        }
        if ((z && cls.isMemberClass() && !Modifier.isStatic(cls.getModifiers())) || !z) {
    ...
    public FieldDeserializer createFieldDeserializer(ParserConfig parserConfig, Class<?> cls, FieldInfo fieldInfo) {
        ...
        boolean z2 = fieldInfo.getFieldClass() != Class.class ? z : false;
        if ((z2 && (aSMDeserializerFactory = this.asmFactory) != null && aSMDeserializerFactory.isExternalClass(cls)) || !z2) {
