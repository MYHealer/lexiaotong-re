...
public class ScanBLEDevice {
    ...
    private ScanBLEDevice() {
        this.mTimeOut = 10000;
        this.mLeScanCallback = new BluetoothAdapter.LeScanCallback() { // from class: com.lxt.bluetoothsdk.bluetooth.ScanBLEDevice.5
            @Override // android.bluetooth.BluetoothAdapter.LeScanCallback
            public void onLeScan(BluetoothDevice bluetoothDevice, int i, byte[] bArr) {
                if (bluetoothDevice == null || TextUtils.isEmpty(bluetoothDevice.getAddress())) {
                    return;
                }
                ScanBLEDevice.this.callOnScan(new com.lxt.bluetoothsdk.model.BluetoothDevice(bluetoothDevice.getName(), bluetoothDevice.getAddress(), i));
            }
        };
    }
    ...
    private void callOnStart() {
        MainThreadExecutor.getInstance().execute(new Runnable() { // from class: com.lxt.bluetoothsdk.bluetooth.ScanBLEDevice.1
    ...
    public void callOnScan(final com.lxt.bluetoothsdk.model.BluetoothDevice bluetoothDevice) {
        MainThreadExecutor.getInstance().execute(new Runnable() { // from class: com.lxt.bluetoothsdk.bluetooth.ScanBLEDevice.2
            @Override // java.lang.Runnable
            public void run() {
                if (!ScanBLEDevice.this.isRunning || ScanBLEDevice.this.mScanListener == null) {
                    return;
    ...
    public void callOnComplete() {
        MainThreadExecutor.getInstance().execute(new Runnable() { // from class: com.lxt.bluetoothsdk.bluetooth.ScanBLEDevice.3
            @Override // java.lang.Runnable
            public void run() {
                try {
                    if (ScanBLEDevice.this.isRunning && ScanBLEDevice.this.mScanListener != null) {
    ...
    private void callOnBluetoothException(final ClientException clientException, final int i) {
        MainThreadExecutor.getInstance().execute(new Runnable() { // from class: com.lxt.bluetoothsdk.bluetooth.ScanBLEDevice.4
            @Override // java.lang.Runnable
            public void run() {
                try {
                    if (ScanBLEDevice.this.isRunning && ScanBLEDevice.this.mScanListener != null) {
