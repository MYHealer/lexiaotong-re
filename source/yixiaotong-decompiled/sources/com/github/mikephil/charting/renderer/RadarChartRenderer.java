...
public class RadarChartRenderer extends LineRadarRenderer {
    ...
    public void drawValues(Canvas canvas) {
        ...
        while (i2 < ((RadarData) this.mChart.getData()).getDataSetCount()) {
            IRadarDataSet dataSetByIndex = ((RadarData) this.mChart.getData()).getDataSetByIndex(i2);
            if (shouldDrawValues(dataSetByIndex)) {
                applyValueTextStyle(dataSetByIndex);
                ValueFormatter valueFormatter = dataSetByIndex.getValueFormatter();
                MPPointF mPPointF3 = MPPointF.getInstance(dataSetByIndex.getIconsOffset());
                ...
                while (i3 < dataSetByIndex.getEntryCount()) {
                    ...
                    float f2 = i3 * sliceAngle * phaseX;
                    Utils.getPosition(centerOffsets, (radarEntry.getY() - this.mChart.getYChartMin()) * factor * phaseY, f2 + this.mChart.getRotationAngle(), mPPointF);
                    if (dataSetByIndex.isDrawValuesEnabled()) {
                        drawValue(canvas, valueFormatter.getRadarLabel(radarEntry), mPPointF.x, mPPointF.y - fConvertDpToPixel, dataSetByIndex.getValueTextColor(i3));
                    }
                    if (radarEntry.getIcon() != null && dataSetByIndex.isDrawIconsEnabled()) {
                        Drawable icon = radarEntry.getIcon();
                        Utils.getPosition(centerOffsets, (radarEntry.getY() * factor * phaseY) + mPPointF4.y, f2 + this.mChart.getRotationAngle(), mPPointF2);
                        mPPointF2.y += mPPointF4.x;
                        Utils.drawImage(canvas, icon, (int) mPPointF2.x, (int) mPPointF2.y, icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
                    }
                    i3++;
                    mPPointF3 = mPPointF4;
