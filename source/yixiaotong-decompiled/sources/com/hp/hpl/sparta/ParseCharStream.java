...
class ParseCharStream implements ParseSource {
    ...
    public ParseCharStream(String str, Reader reader, char[] cArr, ParseLog parseLog, String str2, ParseHandler parseHandler) throws ParseException, IOException {
        ...
        if (str3 != null && !str3.equals(element.getTagName())) {
            parseLog.warning(new StringBuffer("DOCTYPE name \"").append(this.docTypeName_).append("\" not same as tag name, \"").append(element.getTagName()).append("\" of root element").toString(), this.systemId_, getLineNumber());
        }
        while (isMisc()) {
            readMisc();
        }
    ...
    private void readAttribute(Element element) throws ParseException, IOException {
        ...
        if (element.getAttribute(name) != null) {
            this.log_.warning(new StringBuffer("Element ").append(this).append(" contains attribute ").append(name).append("more than once").toString(), this.systemId_, getLineNumber());
        }
    ...
    private char readCharRef() throws ParseException, IOException {
        ...
        try {
            ...
            this.log_.warning(new StringBuffer("\"").append(str).append("\" is not a valid ").append(i == 16 ? "hexadecimal" : "decimal").append(" number").toString(), this.systemId_, getLineNumber());
            return ' ';
        }
    }
    ...
    private void readContent() throws ParseException, IOException {
        ...
        while (z) {
            if (isETag()) {
                ...
                char[] reference = readReference();
                this.handler_.characters(reference, 0, reference.length);
            } else if (isCdSect()) {
                readCdSect();
            } else if (isPi()) {
                readPi();
    ...
    private void readETag(Element element) throws ParseException, IOException {
        ...
        if (!name.equals(element.getTagName())) {
            this.log_.warning(new StringBuffer("end tag (").append(name).append(") does not match begin tag (").append(element.getTagName()).append(")").toString(), this.systemId_, getLineNumber());
        }
        if (isS()) {
            readS();
        }
    ...
    private String readEntityRef() throws ParseException, IOException {
        ...
        if (str2 == null) {
            if (this.isExternalDtd_) {
                parseLog = this.log_;
                stringBufferAppend = new StringBuffer("&").append(name);
                str = "; not found -- possibly defined in external DTD)";
            } else {
                parseLog = this.log_;
                stringBufferAppend = new StringBuffer("No declaration of &").append(name);
                str = ";";
            }
            parseLog.warning(stringBufferAppend.append(str).toString(), this.systemId_, getLineNumber());
            str2 = "";
        }
        readChar(';');
        return str2;
    ...
    private String readPeReference() throws ParseException, IOException {
        ...
        if (str == null) {
            this.log_.warning(new StringBuffer("No declaration of %").append(name).append(";").toString(), this.systemId_, getLineNumber());
            str = "";
        }
        readChar(';');
        return str;
    ...
    private void readPossibleCharData() throws ParseException, IOException {
        ...
        loop0: while (true) {
            i = 0;
            do {
                if (isChar(Typography.less) || isChar(Typography.amp) || isSymbol(END_CDATA)) {
                    break loop0;
                }
                this.tmpBuf_[i] = readChar();
