...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
    ...
    private static long hashLength17to32(byte[] bArr, int i, int i2) {
        ...
        return hashLength16((LittleEndianByteArray.load64(bArr, i3 - 16) * K2) + Long.rotateRight(jLoad64 + jLoad65, 43) + Long.rotateRight(jLoad66, 30), jLoad64 + Long.rotateRight(jLoad65 + K2, 18) + jLoad66, j);
    }
    ...
    private static long hashLength65Plus(byte[] bArr, int i, int i2) {
        ...
        int i8 = i;
        while (true) {
            long jRotateRight = Long.rotateRight(jLoad64 + j + jArr[0] + LittleEndianByteArray.load64(bArr, i8 + 8), 37) * K1;
            long jRotateRight2 = Long.rotateRight(j + jArr[1] + LittleEndianByteArray.load64(bArr, i8 + 48), 42) * K1;
            ...
            weakHashLength32WithSeeds(bArr, i8 + 32, jRotateRight3 + jArr2[1], jLoad65 + LittleEndianByteArray.load64(bArr, i8 + 16), jArr2);
            i8 += 64;
            if (i8 == i4) {
                long j3 = ((j2 & 255) << 1) + K1;
                long j4 = jArr2[0] + ((long) i5);
                jArr2[0] = j4;
                long j5 = jArr[0] + j4;
                jArr[0] = j5;
                jArr2[0] = jArr2[0] + j5;
                long jRotateRight4 = Long.rotateRight(jRotateRight3 + jLoad65 + jArr[0] + LittleEndianByteArray.load64(bArr, i6 - 55), 37) * j3;
                long jRotateRight5 = Long.rotateRight(jLoad65 + jArr[1] + LittleEndianByteArray.load64(bArr, i6 - 15), 42) * j3;
                long j6 = jRotateRight4 ^ (jArr2[1] * 9);
                long jLoad66 = jRotateRight5 + (jArr[0] * 9) + LittleEndianByteArray.load64(bArr, i6 - 23);
                long jRotateRight6 = Long.rotateRight(j2 + jArr2[0], 33) * j3;
                weakHashLength32WithSeeds(bArr, i7, jArr[1] * j3, j6 + jArr2[0], jArr);
                weakHashLength32WithSeeds(bArr, i6 - 31, jRotateRight6 + jArr2[1], LittleEndianByteArray.load64(bArr, i6 - 47) + jLoad66, jArr2);
                return hashLength16(hashLength16(jArr[0], jArr2[0], j3) + (shiftMix(jLoad66) * K0) + j6, hashLength16(jArr[1], jArr2[1], j3) + jRotateRight6, j3);
            }
            jShiftMix = j2;
            j = jLoad65;
