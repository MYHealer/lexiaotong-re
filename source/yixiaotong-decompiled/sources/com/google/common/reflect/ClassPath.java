...
public final class ClassPath {
    ...
    private static final Splitter CLASS_PATH_ATTRIBUTE_SEPARATOR = Splitter.on(PPSLabelView.Code).omitEmptyStrings();
    ...
    public ImmutableSet<ClassInfo> getTopLevelClassesRecursive(String str) {
        Preconditions.checkNotNull(str);
        String string = new StringBuilder(String.valueOf(str).length() + 1).append(str).append(FilenameUtils.EXTENSION_SEPARATOR).toString();
        ImmutableSet.Builder builder = ImmutableSet.builder();
        UnmodifiableIterator<ClassInfo> it = getTopLevelClasses().iterator();
        while (it.hasNext()) {
            ClassInfo next = it.next();
        ...
        for (String str : Splitter.on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
                    ...
                    builder.add(new URL(UriUtil.LOCAL_FILE_SCHEME, (String) null, new File(str).getAbsolutePath()));
                }
