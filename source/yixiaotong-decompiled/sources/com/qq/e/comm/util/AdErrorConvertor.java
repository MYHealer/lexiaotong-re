...
public class AdErrorConvertor {
    ...
    public static AdError formatErrorCode(int i) {
        AdError adError;
        if (i == 4014) {
            adError = new AdError(4014, "广告未成功返回或广告无效时，启动了广告展示，请在广告返回后再进行展示广告的操作。具体方法为，请在show之前，使用isValid判断广告是否有效，当结果为true时再去调用show方法。");
        } else if (i == 4015) {
            adError = new AdError(4015, "同一条广告不允许多次展示，请再次拉取后展示。");
        } else if (i == 5018) {
            adError = new AdError(5018, "当前请求所使用的广告位已被下线，请至开发者平台流量合作板块，检查广告位是否被关闭、冻结或者已被删除。");
        } else if (i == 5019) {
            adError = new AdError(5019, "appid被封，请至平台流量合作板块，检查appid的状态。");
        } else if (i == 5021) {
            ...
            adError = new AdError(5022, "广告模板渲染失败。");
        }
        return adError;
    }
