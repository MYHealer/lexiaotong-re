...
public class Glide implements ComponentCallbacks2 {
    ...
    private static void initializeGlide(Context context, GlideBuilder glideBuilder, GeneratedAppGlideModule generatedAppGlideModule) {
        ...
        if (generatedAppGlideModule == null || generatedAppGlideModule.isManifestParsingEnabled()) {
            listEmptyList = new ManifestParser(origApplicationContext).parse();
        }
        ...
        while (it3.hasNext()) {
            it3.next().applyOptions(origApplicationContext, glideBuilder);
        }
        if (generatedAppGlideModule != null) {
            generatedAppGlideModule.applyOptions(origApplicationContext, glideBuilder);
        }
        Glide glideBuild = glideBuilder.build(origApplicationContext);
        for (GlideModule glideModule : listEmptyList) {
            try {
                glideModule.registerComponents(origApplicationContext, glideBuild, glideBuild.registry);
        ...
        if (generatedAppGlideModule != null) {
            generatedAppGlideModule.registerComponents(origApplicationContext, glideBuild, glideBuild.registry);
        }
        origApplicationContext.registerComponentCallbacks(glideBuild);
        glide = glideBuild;
    }

    private static GeneratedAppGlideModule getAnnotationGeneratedGlideModules(Context context) {
        try {
            return (GeneratedAppGlideModule) Class.forName("com.bumptech.glide.GeneratedAppGlideModuleImpl").getDeclaredConstructor(Context.class).newInstance(StubApp.getOrigApplicationContext(context.getApplicationContext()));
