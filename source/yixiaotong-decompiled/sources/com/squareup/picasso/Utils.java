...
final class Utils {
    ...
    static String createKey(Request request, StringBuilder sb) {
        ...
        if (request.centerCrop) {
            sb.append("centerCrop").append(KEY_SEPARATOR);
    ...
    static boolean parseResponseSourceHeader(String str) {
        ...
        String[] strArrSplit = str.split(PPSLabelView.Code, 2);
        ...
        try {
            return "CONDITIONAL_CACHE".equals(strArrSplit[0]) && Integer.parseInt(strArrSplit[1]) == 304;
    ...
    static File createDefaultCacheDir(Context context) {
        File file = new File(StubApp.getOrigApplicationContext(context.getApplicationContext()).getCacheDir(), PICASSO_CACHE);
        if (!file.exists()) {
            file.mkdirs();
        }
        return file;
    ...
    static long calculateDiskCacheSize(File file) {
        ...
        return Math.max(Math.min(blockCount, a.hnadsb), CacheDataSink.DEFAULT_FRAGMENT_SIZE);
    }
