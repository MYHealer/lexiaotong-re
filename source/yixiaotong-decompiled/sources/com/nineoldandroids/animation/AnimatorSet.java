...
public final class AnimatorSet extends Animator {
    ...
    public void start() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (listeners != null && listeners.size() > 0) {
                for (Animator.AnimatorListener animatorListener : new ArrayList(listeners)) {
        ...
        if (this.mStartDelay <= 0) {
            ...
            valueAnimatorOfFloat.setDuration(this.mStartDelay);
    ...
    public AnimatorSet mo996clone() {
        AnimatorSet animatorSet = (AnimatorSet) super.mo996clone();
        ...
        for (Node node : this.mNodes) {
            Node nodeM997clone = node.m997clone();
            map.put(node, nodeM997clone);
            animatorSet.mNodes.add(nodeM997clone);
            animatorSet.mNodeMap.put(nodeM997clone.animation, nodeM997clone);
            ArrayList arrayList = null;
            nodeM997clone.dependencies = null;
            nodeM997clone.tmpDependencies = null;
            nodeM997clone.nodeDependents = null;
            nodeM997clone.nodeDependencies = null;
            ArrayList<Animator.AnimatorListener> listeners = nodeM997clone.animation.getListeners();
            if (listeners != null) {
                ...
                if (arrayList != null) {
                    Iterator it = arrayList.iterator();
                    while (it.hasNext()) {
                        listeners.remove((Animator.AnimatorListener) it.next());
                    }
                }
            }
        }
        for (Node node2 : this.mNodes) {
            ...
            if (node2.dependencies != null) {
                for (Dependency dependency : node2.dependencies) {
                    node3.addDependency(new Dependency((Node) map.get(dependency.node), dependency.rule));
                }
            }
        }
        return animatorSet;
    ...
    private static class Node implements Cloneable {
        ...
        public Node m997clone() {
            try {
                Node node = (Node) super.clone();
                node.animation = this.animation.mo996clone();
                return node;
