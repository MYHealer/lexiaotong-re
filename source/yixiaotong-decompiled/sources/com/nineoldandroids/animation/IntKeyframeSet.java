...
class IntKeyframeSet extends KeyframeSet {
    ...
    public IntKeyframeSet mo998clone() {
        ArrayList<Keyframe> arrayList = this.mKeyframes;
        int size = this.mKeyframes.size();
        Keyframe.IntKeyframe[] intKeyframeArr = new Keyframe.IntKeyframe[size];
        for (int i = 0; i < size; i++) {
            intKeyframeArr[i] = (Keyframe.IntKeyframe) arrayList.get(i).mo999clone();
        }
    ...
    public int getIntValue(float f) {
        if (this.mNumKeyframes == 2) {
            ...
            return this.mEvaluator == null ? this.firstValue + ((int) (f * this.deltaValue)) : ((Number) this.mEvaluator.evaluate(f, Integer.valueOf(this.firstValue), Integer.valueOf(this.lastValue))).intValue();
        }
        if (f <= 0.0f) {
