...
public class b {
    ...
    public static X509Certificate d(String str) {
        try {
            ...
            g.e(TAG, "generateX509FromStr: CertificateException" + e.getMessage());
            return null;
        }
    }
        ...
        try {
            ...
            while (it.hasNext()) {
                if (arrayList.contains(it.next().getSerialNumber())) {
                    g.e(TAG, "verify: certificate revoked");
                    return false;
                }
            }
            return true;
        } catch (Exception e) {
            g.e(TAG, "verify: revoked verify exception : " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean a(X509Certificate x509Certificate, String str) {
        ...
        g.e(TAG, "verify: subject name is error");
        return false;
    }
    ...
    public static boolean a(X509Certificate[] x509CertificateArr) {
        ...
        for (X509Certificate x509Certificate : x509CertificateArr) {
            try {
                ...
                g.e(TAG, "verifyCertificateDate: exception : " + e.getMessage());
                return false;
            }
        }
        return true;
    ...
    public static boolean a(X509Certificate x509Certificate, X509Certificate x509Certificate2) {
        try {
            ...
            g.e(TAG, "verify: publickey InvalidKeyException " + e.getMessage());
            return false;
        } catch (NoSuchAlgorithmException e2) {
            g.e(TAG, "verify: publickey NoSuchAlgorithmException " + e2.getMessage());
            return false;
        } catch (NoSuchProviderException e3) {
            g.e(TAG, "verify: publickey NoSuchProviderException " + e3.getMessage());
            return false;
        } catch (SignatureException e4) {
            g.e(TAG, "verify: publickey SignatureException " + e4.getMessage());
            return false;
        } catch (CertificateException e5) {
            g.e(TAG, "verify: publickey CertificateException " + e5.getMessage());
            return false;
        }
    }
    ...
    public static boolean a(X509Certificate x509Certificate, X509Certificate[] x509CertificateArr) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException, NoSuchProviderException {
        ...
        byte b = 0;
        while (b < x509CertificateArr.length) {
            X509Certificate x509Certificate2 = x509CertificateArr[b];
            Principal issuerDN = x509Certificate2.getIssuerDN();
            Principal subjectDN = x509Certificate2.getSubjectDN();
            if (principal != null) {
                if (issuerDN.equals(principal)) {
                    x509CertificateArr[b].verify(x509CertificateArr[b - 1].getPublicKey());
                } else {
                    g.e(TAG, "verify: principalIssuer not match");
                    return false;
                }
            }
            b = (byte) (b + 1);
    ...
    public static boolean a(List<X509Certificate> list) {
        for (byte b = 0; b < list.size() - 1; b = (byte) (b + 1)) {
            if (!a(list.get(b))) {
                return false;
            }
        }
