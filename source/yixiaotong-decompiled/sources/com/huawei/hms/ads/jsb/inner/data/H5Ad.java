...
public class H5Ad {
    ...
    public H5Ad(AdContentData adContentData) {
        ...
        this.isSilentReserve = jg.b(adContentData.v());
        ...
        if (!ag.Code(adContentData.o())) {
            ArrayList arrayList = new ArrayList();
            Iterator<String> it = adContentData.o().iterator();
            while (it.hasNext()) {
                arrayList.add(bc.S(it.next()));
            }
        ...
        if (metaDataS != null) {
            this.minEffectiveShowRatio = metaDataS.D();
            this.minEffectiveShowTime = metaDataS.F();
            this.desc = metaDataS.B();
            List<com.huawei.openalliance.ad.beans.metadata.ImageInfo> listS = metaDataS.S();
            if (listS != null && !listS.isEmpty()) {
                this.icon = new ImageInfo(listS.get(0));
            }
            this.imgList = V(metaDataS.d());
            AdSource adSourceCode = AdSource.Code(metaDataS.l());
            if (adSourceCode != null) {
                this.dspName = adSourceCode.Code();
                this.dspLogo = adSourceCode.V();
            }
            this.source = metaDataS.L();
            this.clickBtnTxt = metaDataS.Code();
            this.title = metaDataS.Z();
            this.label = metaDataS.L();
            this.mediaFile = new MediaFile(metaDataS.h(), metaDataS.k());
            this.showAppElement = metaDataS.o();
        }
