...
public class RequestLoggingListener implements RequestListener {
    ...
    public synchronized void onProducerStart(String str, String str2) {
        if (FLog.isLoggable(2)) {
            ...
            long time = getTime();
            this.mProducerStartTimeMap.put(pairCreate, Long.valueOf(time));
    ...
    public synchronized void onProducerFinishWithSuccess(String str, String str2, @Nullable Map<String, String> map) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(str, str2));
            long time = getTime();
            FLog.v(TAG, "time %d: onProducerFinishWithSuccess: {requestId: %s, producer: %s, elapsedTime: %d ms, extraMap: %s}", Long.valueOf(time), str, str2, Long.valueOf(getElapsedTime(lRemove, time)), map);
        }
    }
    ...
    public synchronized void onProducerFinishWithFailure(String str, String str2, Throwable th, @Nullable Map<String, String> map) {
        if (FLog.isLoggable(5)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(str, str2));
            long time = getTime();
            FLog.w(TAG, "time %d: onProducerFinishWithFailure: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s, throwable: %s}", Long.valueOf(time), str, str2, Long.valueOf(getElapsedTime(lRemove, time)), map, th.toString());
        }
    }
    ...
    public synchronized void onProducerFinishWithCancellation(String str, String str2, @Nullable Map<String, String> map) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mProducerStartTimeMap.remove(Pair.create(str, str2));
            long time = getTime();
            FLog.v(TAG, "time %d: onProducerFinishWithCancellation: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s}", Long.valueOf(time), str, str2, Long.valueOf(getElapsedTime(lRemove, time)), map);
        }
    }
    ...
    public synchronized void onRequestSuccess(ImageRequest imageRequest, String str, boolean z) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mRequestStartTimeMap.remove(str);
            long time = getTime();
            FLog.v(TAG, "time %d: onRequestSuccess: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(time), str, Long.valueOf(getElapsedTime(lRemove, time)));
        }
    }
    ...
    public synchronized void onRequestFailure(ImageRequest imageRequest, String str, Throwable th, boolean z) {
        if (FLog.isLoggable(5)) {
            Long lRemove = this.mRequestStartTimeMap.remove(str);
            long time = getTime();
            FLog.w(TAG, "time %d: onRequestFailure: {requestId: %s, elapsedTime: %d ms, throwable: %s}", Long.valueOf(time), str, Long.valueOf(getElapsedTime(lRemove, time)), th.toString());
        }
    }
    ...
    public synchronized void onRequestCancellation(String str) {
        if (FLog.isLoggable(2)) {
            Long lRemove = this.mRequestStartTimeMap.remove(str);
            long time = getTime();
            FLog.v(TAG, "time %d: onRequestCancellation: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(time), str, Long.valueOf(getElapsedTime(lRemove, time)));
        }
    }
