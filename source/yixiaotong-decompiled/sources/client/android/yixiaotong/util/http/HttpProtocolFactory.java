...
public class HttpProtocolFactory {
    ...
    /* JADX INFO: renamed from: client.android.yixiaotong.util.http.HttpProtocolFactory$1, reason: invalid class name */
    static /* synthetic */ class AnonymousClass1 {
    }

    /* synthetic */ HttpProtocolFactory(AnonymousClass1 anonymousClass1) {
        this();
    }
    ...
    private HttpProtocolFactory() {
        ...
        HttpsTrustManager.getInstance().handleSSLHandshake();
        this.mOkClient = new OkClient(okHttpClient);
    }

    private static class SingletonHolder {
        public static final HttpProtocolFactory INSTANCE = new HttpProtocolFactory(null);
    ...
    private static class MyRequestInterceptor implements RequestInterceptor {
        ...
        public void intercept(RequestInterceptor.RequestFacade requestFacade) {
            requestFacade.addHeader("Cookie", LocalDataUtil.getLastTokenInfo(BaseApplication.app));
            requestFacade.addHeader("X-Sid", "2021011300001");
            requestFacade.addHeader("X-Product-Ver", AppUtils.getVersion(BaseApplication.app));
            requestFacade.addHeader("X-Mobile-Info", "Android#?" + AppUtils.getSystemModel() + "#?" + AppUtils.getSystemVersion());
            requestFacade.addHeader("X-clientType", LocalDataUtil.getClientTypeInfo(BaseApplication.app));
            requestFacade.addHeader("X-B3-TraceId", AppUtils.getTraceId());
            requestFacade.addHeader("X-B3-SpanId", AppUtils.getSpanId());
            requestFacade.addHeader("X-Ghost", AppUtils.getGhost());
            requestFacade.addHeader("X-route", LocalDataUtil.getRoute(BaseApplication.app));
        }
    }
}
