...
public class WalletRefundDetailActivity extends BaseActivity {
    ...
    public static void launch(Activity activity, BuyClubCardDetailBean buyClubCardDetailBean) {
        if (activity != null) {
            activity.startActivity(new Intent(activity, (Class<?>) WalletRefundDetailActivity.class).putExtra("bean", buyClubCardDetailBean));
        }
    }
    ...
    public void initView(WalletRefundDetailBean walletRefundDetailBean) {
        ...
        if (walletRefundDetailBean.payFlagType == 1 || walletRefundDetailBean.payFlagType == -1) {
            ...
            this.mTvRefundWay.setText(getString(R.string.platform));
        }
        ...
        if (walletRefundDetailBean.statusFlag == 1) {
            this.mTvState.setText(getString(R.string.studentapply));
        } else if (walletRefundDetailBean.statusFlag == 2) {
            this.mTvState.setText(getString(R.string.revokeapplication));
        } else if (walletRefundDetailBean.statusFlag == 3) {
            this.mTvState.setText(getString(R.string.refusaltorefund));
        } else if (walletRefundDetailBean.statusFlag == 4) {
            this.mTvState.setText(getString(R.string.platformrefund));
        } else if (walletRefundDetailBean.statusFlag == 5) {
            this.mTvState.setText(getString(R.string.merchantmanualrefund));
        }
        if (StringUtils.isNotEmpty(walletRefundDetailBean.extendJson)) {
            try {
                extendJsonBean = (ExtendJsonBean) GsonUtils.getInstance().parse(new TypeToken<ExtendJsonBean>() { // from class: client.android.yixiaotong.v3.ui.order.WalletRefundDetailActivity.1
    ...
    private void getRefundDetail() {
        V3BusinessControllers.getInstance().getRefundIdInfo(getLoginAccount(), UserInfoUtilControl.getInstance().getSchoolId(), this.mBuyClubCardDetailBean.investorId, this.mBuyClubCardDetailBean.orderId, new Listener<WalletRefundDetailBean>() { // from class: client.android.yixiaotong.v3.ui.order.WalletRefundDetailActivity.2
            @Override // client.android.yixiaotong.controller.core.Listener
            public void onStart(Controller controller) {
                BaseMaterialDialog.showMaterialDialog(WalletRefundDetailActivity.this.getActivity(), "正在获取数据", false);
            }

            @Override // client.android.yixiaotong.controller.core.Listener
            public void onComplete(Controller controller, WalletRefundDetailBean walletRefundDetailBean, Object... objArr) {
                if (WalletRefundDetailActivity.this.mIsEnable) {
                    BaseMaterialDialog.dissmisMaterialDialog();
                    WalletRefundDetailActivity.this.initView(walletRefundDetailBean);
                }
            }
            ...
            public void onFail(Controller controller, ClientException clientException) {
                if (WalletRefundDetailActivity.this.mIsEnable) {
                    WalletRefundDetailActivity.this.onError(clientException);
                }
            }
        });
    }
    ...
    public void onError(ClientException clientException) {
        ...
        if (clientException.isNeedTip(clientException.getCode())) {
            SystemErrorTip.getInstance().showTipDialog(getActivity(), new SystemErrorTip.SystemErrorDialogListener() { // from class: client.android.yixiaotong.v3.ui.order.WalletRefundDetailActivity.3
                @Override // client.android.yixiaotong.ui.dialog.SystemErrorTip.SystemErrorDialogListener
                public void onConfirm() {
                    WalletRefundDetailActivity.this.finish();
                }
    ...
    private class RecordsAdapter extends BaseAdapter {
        ...
        @Override // android.widget.Adapter
        public int getCount() {
            return WalletRefundDetailActivity.this.mDataList.size();
        }

        @Override // android.widget.Adapter
        public LogBean getItem(int i) {
            return (LogBean) WalletRefundDetailActivity.this.mDataList.get(i);
        }
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            if (view == null) {
                view = LayoutInflater.from(WalletRefundDetailActivity.this.getContext()).inflate(R.layout.layout_item_refund, (ViewGroup) null);
