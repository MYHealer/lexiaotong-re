...
public class RedPackageDetailActivity extends BaseActivity {
    ...
    public static void launch(Activity activity, CouponBean couponBean, int i) {
        if (activity != null) {
            activity.startActivity(new Intent(activity, (Class<?>) RedPackageDetailActivity.class).putExtra("bean", couponBean).putExtra("state", i));
        }
    }
        ...
        if (this.mCouponBean.typeId != null && this.mCouponBean.typeId.size() > 0) {
            ...
            for (int i4 = 0; i4 < this.mCouponBean.typeId.size(); i4++) {
                for (int i5 = 0; i5 < arrayList.size(); i5++) {
                    Log.e("test", i4 + "  " + i5 + "  " + this.mCouponBean.typeId.get(i4) + "  " + ((TypeInfoBean) arrayList.get(i5)).base.typeId + "  " + sb.toString().contains(((TypeInfoBean) arrayList.get(i5)).base.appName));
