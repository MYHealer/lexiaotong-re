...
public class CachingExec implements ClientExecChain {
    ...
    private CloseableHttpResponse generateCachedResponse(HttpRequestWrapper httpRequestWrapper, HttpContext httpContext, HttpCacheEntry httpCacheEntry, Date date) {
        ...
        if (httpRequestWrapper.containsHeader("If-None-Match") || httpRequestWrapper.containsHeader("If-Modified-Since")) {
            ...
            closeableHttpResponseGenerateNotModifiedResponse = this.responseGenerator.generateResponse(httpCacheEntry);
        }
    ...
    private CloseableHttpResponse unvalidatedCacheHit(HttpContext httpContext, HttpCacheEntry httpCacheEntry) {
        CloseableHttpResponse closeableHttpResponseGenerateResponse = this.responseGenerator.generateResponse(httpCacheEntry);
        setResponseStatus(httpContext, CacheResponseStatus.CACHE_HIT);
    ...
    private String generateViaHeader(HttpMessage httpMessage) {
        ...
        if ("http".equalsIgnoreCase(protocolVersion.getProtocol())) {
            str = String.format("%d.%d localhost (Apache-HttpClient/%s (cache))", Integer.valueOf(protocolVersion.getMajor()), Integer.valueOf(protocolVersion.getMinor()), release);
        } else {
            str = String.format("%s/%d.%d localhost (Apache-HttpClient/%s (cache))", protocolVersion.getProtocol(), Integer.valueOf(protocolVersion.getMajor()), Integer.valueOf(protocolVersion.getMinor()), release);
        }
    ...
    CloseableHttpResponse callBackend(HttpRoute httpRoute, HttpRequestWrapper httpRequestWrapper, HttpClientContext httpClientContext, HttpExecutionAware httpExecutionAware) throws IOException, HttpException {
        ...
        try {
            ...
            return handleBackendResponse(httpRoute, httpRequestWrapper, httpClientContext, httpExecutionAware, currentDate, getCurrentDate(), closeableHttpResponseExecute);
        } catch (IOException e) {
    ...
    CloseableHttpResponse negotiateResponseFromVariants(HttpRoute httpRoute, HttpRequestWrapper httpRequestWrapper, HttpClientContext httpClientContext, HttpExecutionAware httpExecutionAware, Map<String, Variant> map) throws IOException, HttpException {
        ...
        try {
            ...
            if (closeableHttpResponseExecute.getStatusLine().getStatusCode() != 304) {
                return handleBackendResponse(httpRoute, httpRequestWrapper, httpClientContext, httpExecutionAware, currentDate, currentDate2, closeableHttpResponseExecute);
            }
            ...
            closeableHttpResponseExecute.close();
            CloseableHttpResponse closeableHttpResponseGenerateResponse = this.responseGenerator.generateResponse(updatedVariantEntry);
    ...
    CloseableHttpResponse revalidateCacheEntry(HttpRoute httpRoute, HttpRequestWrapper httpRequestWrapper, HttpClientContext httpClientContext, HttpExecutionAware httpExecutionAware, HttpCacheEntry httpCacheEntry) throws IOException, HttpException {
        ...
        if (uri != null) {
            try {
                httpRequestWrapperBuildConditionalRequest.setURI(InternalURIUtils.rewriteURIForRoute(uri, httpRoute));
        ...
        if (revalidationResponseIsTooOld(closeableHttpResponseExecute, httpCacheEntry)) {
            ...
            CloseableHttpResponse closeableHttpResponseExecute2 = this.backend.execute(httpRoute, httpRequestWrapperBuildUnconditionalRequest, httpClientContext, httpExecutionAware);
            date = currentDate4;
            currentDate = getCurrentDate();
            closeableHttpResponse = closeableHttpResponseExecute2;
        } else {
            date = currentDate2;
        ...
        if (statusCode == 304) {
            HttpCacheEntry httpCacheEntryUpdateCacheEntry = this.responseCache.updateCacheEntry(httpClientContext.getTargetHost(), httpRequestWrapper, httpCacheEntry, closeableHttpResponse, date, currentDate);
            ...
            return this.responseGenerator.generateResponse(httpCacheEntryUpdateCacheEntry);
        }
        ...
        return handleBackendResponse(httpRoute, httpRequestWrapperBuildConditionalRequest, httpClientContext, httpExecutionAware, date, currentDate, closeableHttpResponse);
    }

    CloseableHttpResponse handleBackendResponse(HttpRoute httpRoute, HttpRequestWrapper httpRequestWrapper, HttpClientContext httpClientContext, HttpExecutionAware httpExecutionAware, Date date, Date date2, CloseableHttpResponse closeableHttpResponse) throws IOException {
