...
class AsynchronousValidationRequest implements Runnable {
    ...
    protected boolean revalidateCacheEntry() {
        try {
            ...
            try {
                return isNotServerError(closeableHttpResponseRevalidateCacheEntry.getStatusLine().getStatusCode()) && isNotStale(closeableHttpResponseRevalidateCacheEntry);
            } finally {
    ...
    private boolean isNotStale(HttpResponse httpResponse) {
        ...
        for (Header header : headers) {
            ...
            if (value.startsWith(BiddingConst.ADN_ID.SIGMOB) || value.startsWith(BiddingConst.ADN_ID.MM)) {
                return false;
            }
        }
        return true;
