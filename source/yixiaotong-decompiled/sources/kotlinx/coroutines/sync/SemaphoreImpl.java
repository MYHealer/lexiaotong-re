...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2836constructorimpl;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2836constructorimpl = SegmentOrClosed.m2836constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2836constructorimpl = SegmentOrClosed.m2836constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m2841isClosedimpl(objM2836constructorimpl)) {
                Segment segmentM2839getSegmentimpl = SegmentOrClosed.m2839getSegmentimpl(objM2836constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= segmentM2839getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM2839getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM2839getSegmentimpl)) {
                    ...
                    if (segmentM2839getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM2839getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m2839getSegmentimpl(objM2836constructorimpl);
        int i = (int) (andIncrement % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.acquirers, i, null, cont)) {
            cont.invokeOnCancellation(new CancelSemaphoreAcquisitionHandler(semaphoreSegment3, i));
            return true;
        }
        if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM2839getSegmentimpl)) {
                    if (!segment.decPointers$kotlinx_coroutines_core()) {
                        break loop0;
                    }
                    segment.remove();
        ...
        if (andSet == null) {
            ...
            return !Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
    ...
    public final Object acquireSlowPath(Continuation<? super Unit> continuation) {
        ...
        while (!addAcquireToQueue(cancellableContinuationImpl)) {
            if (_availablePermits$FU.getAndDecrement(this) > 0) {
