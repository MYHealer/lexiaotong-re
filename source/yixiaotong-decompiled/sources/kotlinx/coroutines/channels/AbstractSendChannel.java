...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo2788trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m2808successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m2807failurePtdJZtk() : ChannelResult.INSTANCE.m2806closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m2806closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m1281constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m1281constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    public boolean cancel(Throwable cause) {
        ...
        } while (!prevNode.addNext(closed, lockFreeLinkedListHead));
        if (!z) {
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo2832remove()) {
                receive.helpRemove();
            } else {
                objM2824constructorimpl$default = InlineList.m2829plusFjFbRPM(objM2824constructorimpl$default, receive);
            }
        }
        if (objM2824constructorimpl$default != null) {
            if (!(objM2824constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM2824constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM2824constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM2824constructorimpl$default;
    ...
    /* JADX WARN: Code duplicated, block: B:29:0x007f A[EDGE_INSN: B:29:0x007f->B:30:0x0086 BREAK  A[LOOP:0: B:3:0x000b->B:45:?]] */
    /* JADX WARN: Code duplicated, block: B:38:0x009d  */
    /* JADX WARN: Code duplicated, block: B:43:0x0069 A[SYNTHETIC] */
    /* JADX WARN: Code duplicated, block: B:44:0x007b A[SYNTHETIC] */
    /* JADX WARN: Code duplicated, block: B:45:? A[LOOP:0: B:3:0x000b->B:45:?, LOOP_END, SYNTHETIC] */
    public final Object sendSuspend(E e, Continuation<? super Unit> continuation) {
        ...
        while (true) {
            if (isFullImpl()) {
                ...
                if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
                    Result.Companion companion = Result.INSTANCE;
                    cancellableContinuationImpl.resumeWith(Result.m1281constructorimpl(Unit.INSTANCE));
                    break;
                }
                if (objOfferInternal != AbstractChannelKt.OFFER_FAILED) {
                    if (objOfferInternal instanceof Closed) {
