...
public class CamColor {
    ...
    private static CamColor fromJchInFrame(float f, float f2, float f3, ViewingConditions viewingConditions) {
        ...
        return new CamColor(f3, f2, f, c, flRoot, fSqrt, f4, fLog * ((float) Math.cos(d2)), fLog * ((float) Math.sin(d2)));
    }
    ...
    int viewed(ViewingConditions viewingConditions) {
        ...
        float fSignum3 = Math.signum(f7) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(0.0d, (((double) Math.abs(f7)) * 27.13d) / (400.0d - ((double) Math.abs(f7)))), 2.380952380952381d));
