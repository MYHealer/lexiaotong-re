...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initPath(MotionKeyPosition motionKeyPosition, MotionPaths motionPaths, MotionPaths motionPaths2) {
        ...
        float f13 = f4 * f2;
        float f14 = f13 / 2.0f;
        this.mX = (int) ((f6 + f12) - f14);
        float f15 = f * f11;
        float f16 = f5 * f3;
        float f17 = f16 / 2.0f;
        this.mY = (int) ((f8 + f15) - f17);
        this.mWidth = (int) (f7 + f13);
        this.mHeight = (int) (f9 + f16);
        float f18 = Float.isNaN(motionKeyPosition.mPercentY) ? 0.0f : motionKeyPosition.mPercentY;
        this.mMode = 1;
        float f19 = (int) ((motionPaths.mX + f12) - f14);
        this.mX = f19;
        float f20 = (int) ((motionPaths.mY + f15) - f17);
        this.mX = f19 + ((-f11) * f18);
        this.mY = f20 + (f10 * f18);
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(motionKeyPosition.mTransitionEasing);
    ...
    void setView(float f, MotionWidget motionWidget, int[] iArr, double[] dArr, double[] dArr2, double[] dArr3) {
        ...
        if (motion != null) {
            ...
            f2 = f7;
            if (!Float.isNaN(f15)) {
                motionWidget.setRotationZ(((float) (((double) f15) + Math.toDegrees(Math.atan2(f10 + (f12 / 2.0f), f9 + (f11 / 2.0f))))) + 0.0f);
            }
        }
        float f20 = f4 + 0.5f;
        float f21 = f5 + 0.5f;
